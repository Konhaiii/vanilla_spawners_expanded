...
public class ModBlocks {
	...
	public static Block register(Block block, String name, boolean shouldRegisterItem) {
		...
		if (shouldRegisterItem) {
			BlockItem blockItem = new BlockItem(block, new Item.Settings().rarity(Rarity.EPIC).component(DataComponentTypes.BLOCK_ENTITY_DATA, CalibratedSpawnerBlockEntity.getDefaultComponent()));
	...
	private static <T extends BlockEntity> BlockEntityType<T> register(BlockEntityType.BlockEntityFactory<? extends T> entityFactory) {
		...
		return Registry.register(Registries.BLOCK_ENTITY_TYPE, id, BlockEntityType.Builder.<T>create(entityFactory, new Block[]{ModBlocks.CALIBRATED_SPAWNER}).build());
	}
